...
def test_results_manifest_rejects_tampered_results(tmp_path):
    ...
    manifest={
        ...
        "policy":_policy_provenance(),"runtime":_runtime_provenance(),
        "status":"complete",
...
def test_results_manifest_rejects_missing_or_invalid_code_revision(tmp_path):
    dataset=tmp_path / "dataset.jsonl"
    dataset.write_text('{"text":"one","expected":"fast","ood":false}\n',encoding="utf-8")
    results=tmp_path / "results.jsonl"
    results.write_text('{"text":"one"}\n',encoding="utf-8")
    ...
    cases=(
        ("missing_revision",None,None),
        ("missing_dirty",None,None),
        ("invalid_revision","NOT-A-SHA",None),
        ("null_revision_dirty",None,False),
        ("sha_without_dirty","a" * 40,None),
        ("sha_bad_dirty","a" * 40,"no"),
    )
    for mode,revision,dirty in cases:
        manifest=dict(base)
        if mode == "missing_revision":
            manifest.pop("code_revision")
        elif mode == "missing_dirty":
            manifest.pop("code_dirty")
        else:
            manifest["code_revision"]=revision
            manifest["code_dirty"]=dirty
        Path(str(results)+".manifest.json").write_text(json.dumps(manifest),encoding="utf-8")
        try:
            validate_results_manifest(results,[str(dataset)])
            assert False,mode
...
def test_results_manifest_rejects_invalid_runtime_provenance(tmp_path):
    dataset=tmp_path / "dataset.jsonl"
    dataset.write_text('{"text":"one","expected":"fast","ood":false}\n',encoding="utf-8")
    results=tmp_path / "results.jsonl"
    results.write_text('{"text":"one"}\n',encoding="utf-8")
    invalid_runtimes=(
        {"max_questions":32},
        {**_runtime_provenance(),"max_questions":0},
        {**_runtime_provenance(),"max_output_tokens":"1024"},
        {**_runtime_provenance(),"max_total_output_tokens":1024},
        {**_runtime_provenance(),"call_timeout_seconds":0},
        {**_runtime_provenance(),"request_timeout_seconds":float("inf")},
    )
