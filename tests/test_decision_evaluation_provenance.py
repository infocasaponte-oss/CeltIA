...
def test_results_manifest_matches_exact_dataset_evidence(tmp_path):
    ...
    manifest={"format_version":RESULT_FORMAT_VERSION,"code_revision":None,"status":"complete","collected_at":"2026-01-01T00:00:00+00:00","datasets":[str(dataset)],"selection":_selection(),"backend":_backend_provenance(),"policy":_policy_provenance(),"dataset_sha256":dataset_sha256([str(dataset)]),"results_sha256":file_sha256(results),"result_rows":1}
...
def test_results_manifest_rejects_tampered_results(tmp_path):
    ...
    results.write_text('{"text":"one"}\n',encoding="utf-8")
    manifest={
        "format_version":RESULT_FORMAT_VERSION,"code_revision":None,
...
def test_results_manifest_rejects_missing_or_invalid_code_revision(tmp_path):
    ...
    for mode,value in (("missing",None),("invalid","NOT-A-SHA")):
        manifest=dict(base)
        if mode == "missing":
            manifest.pop("code_revision")
        else:
            manifest["code_revision"]=value
        Path(str(results)+".manifest.json").write_text(json.dumps(manifest),encoding="utf-8")
        try:
            validate_results_manifest(results,[str(dataset)])
            assert False,mode
