...
Resume is provenance-safe: if a result file already exists, `--resume` now requires its sidecar manifest and verifies the dataset digest plus backend and policy identity against the current run before reusing any row. Missing, incompatible, changed-dataset, changed-backend, or changed-policy provenance fails closed. A successful resumed manifest records the original collection timestamp in `resumed_from_collected_at`.

Promotion evaluation now enforces the same evidence boundary: whenever `--cde-results` is supplied, the evaluator requires the adjacent version-3 manifest and recomputes the digest of the selected datasets before calculating metrics or eligibility. Results collected against different or subsequently modified benchmark evidence are rejected rather than scored.
