...
- API schema and runtime both enforce strict non-blank string fields, integer score bounds, bounded request/question/option sizes plus full per-type semantics (choice cardinality/uniqueness and score ranges with 2..101 candidate values); invalid definitions/output fail closed;
