...
- request-cost bounds are operator-configurable: maximum questions per request (hard-capped at 32), model output tokens per scoring call (64..2048), an aggregate output-token budget divided across all questions, and an aggregate serialized prompt-size budget checked before any model call;
